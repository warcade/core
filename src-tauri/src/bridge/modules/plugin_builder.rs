...
//! It's part of the core bridge (not a plugin) to avoid the chicken-egg problem
//! where the developer plugin would need to rebuild itself.
...
"#, plugin_struct, handler_wrappers);
    ...
    fn bundle_frontend(&self, logs: &mut Vec<BuildLog>) -> Result<()> {
        ...
        let possible_paths: Vec<PathBuf> = [
            ...
            exe_dir.as_ref().map(|p| p.join("scripts").join("build_plugin.js")),
            // Production: bundled as Tauri resource (macOS .app bundle)
            exe_dir.as_ref().map(|p| p.join("../Resources/scripts/build_plugin.js")),
            // Dev mode: src-tauri/scripts
            Some(self.project_root.join("scripts").join("build_plugin.js")),
            // Dev mode: scripts folder next to src-tauri
            self.project_root.parent()
                .map(|p| p.join("src-tauri").join("scripts").join("build_plugin.js")),
        ]
        .into_iter()
        .flatten()
        .collect();
        ...
        let bundler_script = possible_paths
            .iter()
            .find(|p| p.exists())
            .cloned();

        let bundler_script = match bundler_script {
            Some(path) => {
                logs.push(BuildLog {
                    log_type: "info".to_string(),
