...
// Find the main project's node_modules (webarcade/node_modules)
// Script could be at:
//   - Dev: webarcade/src-tauri/builder/scripts/build_plugin.js
//   - Build: webarcade/src-tauri/target/debug/builder/scripts/build_plugin.js
// We need to find webarcade/node_modules reliably
function findMainNodeModules() {
  let dir = __dirname;
  // Walk up until we find a directory with node_modules containing @tabler/icons-solidjs
  for (let i = 0; i < 10; i++) {
    const nodeModules = resolve(dir, 'node_modules');
    const tablerIcons = resolve(nodeModules, '@tabler', 'icons-solidjs');
    if (fs.existsSync(tablerIcons)) {
      return nodeModules;
    }
  ...
  // Fallback: assume we're in src-tauri/builder/scripts
  return resolve(__dirname, '..', '..', '..', '..', 'node_modules');
}

const mainNodeModules = findMainNodeModules();
console.log(`   📁 Using node_modules from: ${mainNodeModules}`);
...
`;
  ...
  const config = {
    ...
    // Note: @tabler/icons-solidjs is NOT external - plugins bundle their own icons (tree-shaken)
    externals: {
      'solid-js': 'SolidJS',
    ...
    resolve: {
      ...
      modules: [
        ...
        mainNodeModules,                      // Main project's node_modules (has icons, etc.)
        'node_modules'                        // Standard node resolution
      ]
    },
