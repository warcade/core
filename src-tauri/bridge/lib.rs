...
use crate::core::{EventBus, WebSocketBridge, RouterRegistry, DynamicPluginLoader};
...
pub async fn run_server() -> Result<()> {
    ...
    // Set global event bus for system-wide event publishing (used by builder, etc.)
    crate::core::set_global_event_bus(event_bus.clone());
    ...
    crate::core::plugin_exports::set_global_router_registry(router_registry.clone_registry());
    ...
    // For exported apps, check for bundled plugins in the resources directory
    // This is relative to the executable location
    let exe_dir = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|p| p.to_path_buf()));

    let bundled_plugins_dir = exe_dir.map(|dir| dir.join("plugins"));

    let mut dynamic_loader = if let Some(ref bundled_dir) = bundled_plugins_dir {
        info!("📦 Checking for bundled plugins at: {:?}", bundled_dir);
        DynamicPluginLoader::new(plugins_dir.clone()).with_bundled_dir(bundled_dir.clone())
    } else {
        DynamicPluginLoader::new(plugins_dir.clone())
    };
        ...
        Ok(dynamic_plugins) => {
            ...
            for plugin_info in &dynamic_plugins {
                ...
                if !plugin_info.routes.is_empty() {
                    ...
                    let mut plugin_router = crate::core::PluginRouter::new();
                        ...
                        if let (Some(method_str), Some(path), Some(handler_name)) = (
                            ...
                            plugin_router.route(method, path, move |path_arg, query, req| {
                                ...
                                Box::pin(async move {
                                    ...
                                    let libs = crate::core::plugin_exports::PLUGIN_LIBRARIES.lock().unwrap();

                                    if let Some(lib) = libs.get(&plugin_id) {
                                        // Get the shared runtime pointer
                                        let runtime_ptr = crate::core::plugin_exports::get_shared_runtime_ptr();
...
async fn handle_request(req: Request<Incoming>, router_registry: RouterRegistry) -> Response<BoxBody<Bytes, Infallible>> {
    ...
    // Dev-tools only endpoints (builder and developer API)
    #[cfg(feature = "dev-tools")]
    {
        // Plugin build endpoint with real-time log streaming via WebSocket
        if path.starts_with("/api/build/") && method == hyper::Method::POST {
            let plugin_id = &path[11..]; // "/api/build/".len() = 11
            if !plugin_id.is_empty() {
                let plugin_id_for_build = plugin_id.to_string();
                let plugin_id_for_callback = plugin_id.to_string();

                // Create callback that publishes build logs to EventBus
                let callback: modules::plugin_builder::LogCallback = Arc::new(move |log| {
                    crate::core::publish_event(
                        "builder",
                        "build.log",
                        serde_json::json!({
                            "plugin_id": plugin_id_for_callback,
                            "log": log
                        })
                    );
                });

                // Run the build in a blocking thread to allow async tasks (WebSocket) to process
                let result = tokio::task::spawn_blocking(move || {
                    modules::plugin_builder::build_plugin_with_callback(&plugin_id_for_build, callback)
                }).await.unwrap_or_else(|e| modules::plugin_builder::BuildResult {
                    success: false,
                    output_path: None,
        ...
        // Toolchain check endpoint
        if path == "/api/toolchain/check" && method == hyper::Method::GET {
            return modules::developer_api::handle_check_toolchain();
        }

        // Install Rust endpoint
        if path == "/api/toolchain/install-rust" && method == hyper::Method::POST {
            let callback: modules::developer_api::InstallLogCallback = Arc::new(move |log| {
                crate::core::publish_event(
                    "installer",
                    "install.log",
            ...
            let result = tokio::task::spawn_blocking(move || {
                modules::developer_api::install_rust(callback)
            }).await.unwrap_or_else(|e| modules::developer_api::InstallResult {
                success: false,
        ...
        // Install MSVC endpoint
        if path == "/api/toolchain/install-msvc" && method == hyper::Method::POST {
            let callback: modules::developer_api::InstallLogCallback = Arc::new(move |log| {
                crate::core::publish_event(
                    "installer",
                    "install.log",
            ...
            let result = tokio::task::spawn_blocking(move || {
                modules::developer_api::install_msvc(callback)
            }).await.unwrap_or_else(|e| modules::developer_api::InstallResult {
                success: false,
                message: "Install task panicked".to_string(),
                error: Some(format!("{:?}", e)),
        ...
        // Export app endpoint with real-time log streaming
        if path == "/api/export" && method == hyper::Method::POST {
            use http_body_util::BodyExt;
            let body_bytes = match req.collect().await {
                Ok(collected) => collected.to_bytes(),
                Err(_) => return Response::builder()
            ...
            let request: modules::developer_api::ExportAppRequest = match serde_json::from_slice(&body_bytes) {
                Ok(r) => r,
                Err(e) => return Response::builder()
                    .status(StatusCode::BAD_REQUEST)
                    .header("Content-Type", "application/json")
            ...
            // Create callback that publishes export logs to EventBus
            let callback: modules::developer_api::ExportLogCallback = Arc::new(move |log| {
                crate::core::publish_event(
                    "exporter",
                    "export.log",
                    serde_json::json!({
            ...
            // Run the export in a blocking thread
            let result = tokio::task::spawn_blocking(move || {
                modules::developer_api::run_export_build(request, callback)
            }).await.unwrap_or_else(|e| modules::developer_api::ExportResult {
                success: false,
                output_path: None,
            ...
            let json = serde_json::to_string(&result).unwrap_or_else(|_| {
                r#"{"success":false,"error":"Failed to serialize export result"}"#.to_string()
            });
            return Response::builder()
                .status(if result.success { StatusCode::OK } else { StatusCode::INTERNAL_SERVER_ERROR })
                .header("Content-Type", "application/json")
