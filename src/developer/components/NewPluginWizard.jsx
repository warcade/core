...
import { IconX, IconCheck, IconFile, IconBrandRust, IconRocket } from '@tabler/icons-solidjs';
...
export function NewPluginWizard(props) {
  ...
  const templates = [
    ...
    },
    {
      ...
      description: 'Complete plugin with Rust backend and frontend',
      icon: IconRocket,
      files: ['mod.rs', 'router.rs', 'index.jsx', 'viewport.jsx', 'api.js', 'package.json'],
    },
  ];
  ...
  const createPlugin = async () => {
    setCreating(true);
    try {
      const response = await api('developer/create', {
        method: 'POST',
        headers: {
      ...
      window.dispatchEvent(new CustomEvent('developer:plugin-created', {
